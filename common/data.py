...
class CustomGraphDataset:
    ...
    def _build_graph(self):
        ...
        for node in G.nodes():
            ...
            if "label" not in attrs:
                attrs["label"] = attrs.get("title", f"Product {node}")
    ...
    def gen_batch(self, batch_size, *args, train=True, **kwargs):
        ...
        while len(neg_a) < batch_size // 2 and tries < max_tries:
            ...
            if (
                sub_a.number_of_edges() > 0
                and sub_b.number_of_edges() > 0
                and not nx.is_isomorphic(sub_a, sub_b)
            ):
...
class OTFSynDataSource(DataSource):
    ...
    def gen_data_loaders(
        ...
        for i in range(2):
            dataset = combined_syn.get_dataset(
                "graph", size // 2, np.arange(self.min_size + 1, self.max_size + 1)
            )
            sampler = (
            ...
            loaders.append(
                TorchDataLoader(
                    ...
                    batch_size=batch_size // 2 if i == 0 else batch_size // 2,
                    sampler=sampler,
                    shuffle=False,
                )
            )
    ...
    def gen_batch(self, batch_target, batch_neg_target, batch_neg_query, train):
        def sample_subgraph(
            ...
            use_hard_neg = (
                hard_neg_idxs is not None and graph.G.graph["idx"] in hard_neg_idxs
            )
            done = False
            n_tries = 0
            while not done:
                if use_precomp_sizes:
                    ...
                    if train and supersample_small_graphs:
                        ...
                        d = 1 if train else 0
                        size = random.randint(
                            self.min_size + offset - d, len(graph.G) - 1 + offset
                        )
                    ...
                    if random.random() < 1.0 or not self.node_anchored:  # add edges
                        ...
                        if len(non_edges) > 0:
                            for u, v in random.sample(
                                non_edges, random.randint(1, min(len(non_edges), 5))
                            ):
                                neigh.add_edge(u, v)
                    else:  # perturb anchor
                        anchor = random.choice(list(neigh.nodes))
                        for v in neigh.nodes:
                            neigh.nodes[v]["node_feature"] = (
                                torch.ones(1) if anchor == v else torch.zeros(1)
                            )
        ...
        # TODO: use hard negs
        hard_neg_idxs = set(
            random.sample(range(len(neg_target.G)), int(len(neg_target.G) * 1 / 2))
        )
        # hard_neg_idxs = set()
        batch_neg_query = Batch.from_data_list(
            [
                DSGraph(
                    self.generator.generate(size=len(g))
    ...
    def gen_batch(self, graphs_a, graphs_b, _, train):
        def add_anchor(g):
            ...
            for v in g.G.nodes:
                g.G.nodes[v]["node_feature"] = (
                    torch.ones(1)
                    if anchor == v or not self.node_anchored
                    else torch.zeros(1)
                )
            return g

        pos_a, pos_b, neg_a, neg_b = [], [], [], []
        fn = "data/cache/imbalanced-{}-{}".format(
...
class DiskDataSource(DataSource):
    ...
    def __init__(self, dataset_name, node_anchored=False, min_size=5, max_size=29):
        self.node_anchored = node_anchored
    ...
    def gen_batch(
        ...
        for i in range(batch_size // 2):
            if sample_method == "tree-pair":
                ...
                a = graph.nodes
                _, b = utils.sample_neigh(
                    [graph], random.randint(min_size, len(graph) - 1)
                )
            if self.node_anchored:
        ...
        while len(neg_a) < batch_size // 2:
            if sample_method == "tree-pair":
                ...
                a = graph_a.nodes
                graph_b, b = utils.sample_neigh(
                    graphs, random.randint(min_size, len(graph_a) - 1)
                )
            if self.node_anchored:
...
class DiskImbalancedDataSource(OTFSynDataSource):
    ...
    def gen_batch(self, graphs_a, graphs_b, _, train):
        ...
        pos_a, pos_b, neg_a, neg_b = [], [], [], []
        fn = "data/cache/imbalanced-{}-{}-{}".format(
            self.dataset_name.lower(), str(self.node_anchored), self.batch_idx
        )
        if not os.path.exists(fn):
            graphs_a = graphs_a.apply_transform(add_anchor)
            graphs_b = graphs_b.apply_transform(add_anchor)
...
if __name__ == "__main__":
    ...
    for name in ["enzymes", "reddit-binary", "cox2"]:
        ...
        clustering = [
            nx.average_clustering(graph.subgraph(nodes)) for graph, nodes in neighs
        ]
        path_length = [
