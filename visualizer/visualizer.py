...
class GraphDataExtractor:
    ...
    def _get_node_positions(self, graph: nx.Graph) -> Dict[str, Tuple[float, float]]:
        ...
        has_positions = all(
            "x" in graph.nodes[n] and "y" in graph.nodes[n] for n in graph.nodes()
        )

        if has_positions:
            return {
                n: (graph.nodes[n]["x"], graph.nodes[n]["y"]) for n in graph.nodes()
            }
    ...
    def _generate_legend(
        ...
        return {"nodeTypes": node_legend, "edgeTypes": edge_legend}
...
class HTMLTemplateProcessor:
    ...
    def read_template(self) -> str:
        ...
        try:
            ...
            if not self._validate_template_structure(content):
                raise ValueError(
                    f"Template file {self.template_path} is missing required structure"
                )
            ...
            raise IOError(
                f"Failed to read template file {self.template_path}: {str(e)}"
            )
    ...
    def inject_graph_data(
        self, template_content: str, graph_data: Dict[str, Any]
    ) -> str:
        """
        ...
        try:
            ...
            if re.search(pattern, template_content, re.DOTALL):
                ...
                if re.search(simple_pattern, template_content, re.DOTALL):
                    injected_content = re.sub(
                        simple_pattern, replacement, template_content, flags=re.DOTALL
                    )
                else:
                    raise RuntimeError(
                        "Could not find GRAPH_DATA placeholder in template"
                    )

            # Verify injection was successful
            if "const GRAPH_DATA" not in injected_content:
                raise RuntimeError(
                    "Data injection failed - GRAPH_DATA not found in result"
                )
    ...
    def generate_filename(
        ...
        try:
            ...
            timestamp = (
                int(time.time()) if "time" in globals() else random.randint(1000, 9999)
            )
    ...
    def write_html_file(
        self, content: str, filename: str, output_dir: str = "."
    ) -> str:
        """
    ...
    def process_template(
        ...
        try:
            ...
            output_path = self.write_html_file(
                injected_content, output_filename, output_dir
            )
...
def visualize_pattern_graph_ext(pattern, args, count_by_size, node_labels=None):
    ...
    try:
        ...
        if not isinstance(
            pattern, (nx.Graph, nx.DiGraph, nx.MultiGraph, nx.MultiDiGraph)
        ):
        ...
        logger.info(
            f"Processing {graph_type} graph with {num_nodes} nodes and {num_edges} edges"
        )
        ...
        try:
            ...
            output_dir = os.path.abspath(
                os.path.join(os.path.dirname(__file__), "..", "plots/cluster")
            )
        ...
        if count_by_size is not None and isinstance(count_by_size, dict):
            try:
                ...
                if num_nodes in count_by_size:
                    ...
                    logger.info(
                        f"Graph size {num_nodes} has {count_by_size[num_nodes]} occurrences"
                    )
                else:
