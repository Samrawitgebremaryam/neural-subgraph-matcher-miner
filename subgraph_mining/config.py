...
def parse_decoder(parser):
    ...
    dec_parser.add_argument(
        "--sample_method", type=str, help='"tree" or "radial" sampling method'
    )
    dec_parser.add_argument("--radius", type=int, help="radius of node neighborhoods")
    dec_parser.add_argument(
        "--subgraph_sample_size",
        type=int,
    ...
    dec_parser.add_argument(
        "--min_pattern_size", type=int, help="minimum size of patterns to find"
    )
    dec_parser.add_argument(
        "--max_pattern_size", type=int, help="maximum size of patterns to find"
    )
    dec_parser.add_argument(
    ...
    dec_parser.add_argument(
        "--n_neighborhoods", type=int, help="number of neighborhoods to sample"
    )

    # Search strategy parameters
    dec_parser.add_argument(
        "--search_strategy", type=str, help='"greedy" or "mcts" search strategy'
    )
    dec_parser.add_argument(
        "--n_trials", type=int, help="number of search trials to run"
    )
    dec_parser.add_argument(
        "--out_batch_size", type=int, help="number of motifs to output per graph size"
    )
    ...
    parser.add_argument(
        "--beam_width", type=int, default=5, help="Width of beam for beam search"
    )
    # Output and analysis
    dec_parser.add_argument(
        "--out_path", type=str, help="path to output candidate motifs"
    )
    dec_parser.add_argument(
        "--analyze", action="store_true", help="enable analysis mode"
    )
    dec_parser.add_argument("--motif_dataset", type=str, help="Motif dataset to use")
    dec_parser.add_argument(
        "--n_clusters", type=int, help="number of clusters for analysis"
    )
